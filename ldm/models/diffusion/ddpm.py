...
class LatentDiffusionSRTextWT(DDPM):
    ...
    def __init__(self,
        ...
        if ship_classifier_config is not None:
            self.instantiate_shipclassifier_stage(ship_classifier_config)
        ...
        self.clip_denoised = False
        self.bbox_tokenizer = None
        self.guidance_loss_scale = guidance_loss_scale
    ...
    def instantiate_shipclassifier_stage(self, config):
        ...
        for param in model.fc.parameters():
            param.requires_grad = True
        model.load_state_dict(torch.load(config.params.ckpt_path))
        self.ship_classifier = model
        #self.ship_classifier.train()
        self.ship_classifier.eval()
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if hasattr(self, "dwt"):
            ...
            x_lq_wav = torch.cat([xll, xlh, xhl, xhh], dim=1)  # [b, 12, h, w] 
    ...
    def shared_step(self, batch, **kwargs):
        x, c, gt, classes_embed = self.get_input(batch, self.first_stage_key)
        loss = self(x, c, gt, classes_embed)
        return loss

    def forward(self, x, c, gt, classes_embed, *args, **kwargs):
        ...
        if self.test_gt:
            ...
            struc_c = self.structcond_stage_model(classes_embed, t_ori, x) #here x is the latent for wavediff but for us its the latent of the low quality image
        else:
            struc_c = self.structcond_stage_model(x, t_ori, classes_embed) #classes_embed = wavelet if not classifier
    ...
    def p_losses(self, x_start, cond, struct_cond, t, t_ori, z_gt, noise=None):
        ...
        # if self.guidance_loss_scale != 0 and model_output.grad_fn is not None:
        #     # Calculate aesthetic loss
        #     loss_ae = self.ship_classifier.calculate_loss(
        #                 self.model.diffusion_model.middle_block.output
        #                 )

        #     # Get gradient
        #     cond_grad = torch.autograd.grad(loss_ae*self.guidance_loss_scale, latents)[0]

        #     # Modify the latents based on this gradient
        #     model_output_ = latents.detach() - cond_grad # * sigma**2 
            ...
            if t_replace is not None:
                if start_T is not None:
                    if self.ori_timesteps[i] > start_T:
                         continue
                if hasattr(self, "dwt"):
                    struct_cond_input = self.structcond_stage_model(classes_embed, t_replace, struct_cond) #here x is the latent for wavediff but for us its the latent of the low quality image
                else:
                    struct_cond_input = self.structcond_stage_model(struct_cond, t_replace, classes_embed)
            else:
                if start_T is not None:
                    if i > start_T:
                        continue
                if hasattr(self, "dwt"):
                    struct_cond_input = self.structcond_stage_model(classes_embed, ts, struct_cond) #here x is the latent for wavediff but for us its the latent of the low quality image
                else:
                    struct_cond_input = self.structcond_stage_model(struct_cond, ts, classes_embed)
    ...
    def sample(self, cond, struct_cond, batch_size=16, return_intermediates=False, x_T=None,
               ...
               mask=None, x0=None, shape=None, time_replace=None, adain_fea=None, interfea_path=None,
                 start_T=None, classes_embed=None, **kwargs):
